...
import jwt
from datetime import timedelta # Añade timedelta a tus importaciones de datetime

# Configuración JWT (En producción esto debe ir en un archivo .env)
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> UserInDB:
    ...
    try:
        # Decodificamos el JWT matemáticamente
...
def require_admin(user: UserInDB = Depends(get_current_user)) -> UserInDB:
    # Verificamos si tiene el rol 1 (Admin/Gestor)
    if 1 not in user.role_ids:
...
def login(payload: LoginRequest) -> TokenResponse:
    ...
    # Generamos el JWT real con los roles dentro
    expire = datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode = {
        "sub": str(user.id), 
        "exp": expire, 
        "roles": user.role_ids # Guardamos los roles en el token
    }
...
def create_user_alert(user_id: int, payload: AlertCreate, _: UserInDB = Depends(require_admin)) -> Alert:
    ensure_user_exists(user_id)
